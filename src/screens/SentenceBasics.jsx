...
import { SENTENCE_BASICS } from "../data";

// Read-through explainer opened from the Sentences card. Full-screen overlay.
export default function SentenceBasics({ onClose }) {
  const { t } = useTheme();
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1500, background: t.bg, display: "flex", flexDirection: "column",
      ...
      <header style={{ display: "flex", alignItems: "center", gap: 12, padding: "calc(14px + env(safe-area-inset-top)) 20px 12px" }}>
        ...
        <h1 style={{ margin: 0, fontSize: 19, fontWeight: 800, letterSpacing: "-0.02em", color: t.ink }}>Sentence basics</h1>
      </header>

      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "4px 20px 28px" }}>
        <p style={{ margin: "0 0 16px", fontSize: 14, color: t.sub, fontWeight: 600, lineHeight: 1.5 }}>
          A quick tour of how Japanese sentences fit together, before you start the Sentences chapter.
        </p>
        <div style={{ display: "grid", gap: 12 }}>
          {SENTENCE_BASICS.map((c, i) => (
