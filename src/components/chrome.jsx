...
export function Cat({ mood = "general", size = 64, style }) {
  return (
...
export function Ring({ size = 46, stroke = 4, pct, color, track, children }) {
    ...
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth={stroke} strokeLinecap="round"
          ...
          style={{ transition: "stroke-dasharray 600ms cubic-bezier(.3,.8,.3,1)" }} />
      </svg>
  ...
  return (
    <div style={{ flexShrink: 0, background: t.surface, borderTop: `1px solid ${t.line}`, paddingBottom: "env(safe-area-inset-bottom)" }}>
      <div style={{ display: "flex" }}>
        {["Learn", "Practice", "Profile"].map((label) => {
...
export function Modal({ children, onDismiss, position = "center" }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", justifyContent: "center",
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 402 }}>{children}</div>
    </div>
  );
}

// App shell — fills the viewport, phone-width on larger screens.
export function Shell({ children, active = "Learn", onNav, nav = true }) {
  ...
  const shadowColor = mode === "dark" ? "rgba(0,0,0,0.4)" : "rgba(30,37,64,0.13)";
  return (
    <div style={{ width: "100%", maxWidth: 430, margin: "0 auto", height: "100dvh", background: t.bg,
      ...
      <div ref={ref} style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden",
        ...
        <div aria-hidden style={{ position: "absolute", top: "env(safe-area-inset-top)", left: 0, right: 0, height: 12,
          pointerEvents: "none", zIndex: 5, transition: "opacity 220ms ease", opacity: scrolled ? 1 : 0,
